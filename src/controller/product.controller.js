...
import { SendCreate, SendError, SendError400, SendSuccess } from "../service/response";
import { ValidateData } from "../service/validate";
export default class ProductController {
    static async getAll(req, res) {
        try {
            const category = `Select * from product INNER JOIN category on categoryID = product.categoryID`;
    ...
    static async getOne(req, res) {
        try {
            const uuid = req.params.cUuid; // 1 body: {} 2 params ແມ່ນ string ທີ່ຈະຢູ່ກັບ url , 3 query ຄ້າຍຄືກັບ params ແຕ່ສາມາດຂຽນຂໍ້ມູນໃນ url ໄດ້
            const checkUuid = `select * from product INNER JOIN category on categoryID = product.categoryID Where PUuid=?`;
    ...
    static async insert(req, res) {
        try {
            ...
            const validate = await ValidateData({ name, detail, amount, price, categoryID });
            ...
            connected.query(checkCategory, categoryID, async (err, result) => {
                ...
                if (!result[0]) return SendError(res, 404, EMessage.NotFound, "category");
                const image = req.files;
                if (!image) return SendError400(res, EMessage.BadRequest + "Image")
                const image_url = await UploadImageToCloud(image.image.deta);
                ...
                const datetime = new Date()
                    ...
                    .replace(/T/, " ")
                    .replace(/\..+/, "");
                const insert = `insert into product (pUuid,name,detail,amount,price,categoryID,image,createdAt,updatedAt)
                values(?,?,?,?,?,?,?,?,?)`;
                connected.query(insert, [pUuid, name, detail, amount, price, categoryID, image_url, datetime, datetime], (error) => {
                    if (error) return SendError(res, 404, EMessage.ErrorInsert, err);
                    return SendCreate(res, SMessage.Insert);
                });
            });
        } catch (error) {
            return SendError(res, SMessage.SendError, error);
        }
    }
        ...
        try {
            ...
            connected.query(checkCategory, categoryID, async (err, result) => {
                ...
                const datetime = new Date()
                    ...
                    .replace(/T/, " ")
                    .replace(/\..+/, "");
                const update = `update product set name=? ,detail=? ,amount=?, price=? ,categoryID=? image=? ,updatedAt=? where pUuid=?`;
                connected.query(update, [name, detail, amount, price, categoryID, image_url, datetime, pUuid], (error) => {
